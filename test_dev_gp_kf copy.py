...
class MinT(Kernel):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if Y is None:
            ...
            K = np.min(Kcr, axis=-1) * self.sigma_0
        else:
        ...
        if eval_gradient:
            if Y is None:
                gradients = np.sum(self.sigma_0*np.abs(X),1)[:,None]
            else:
                gradients = np.sum(np.where(np.logical_and(np.abs(X)<np.abs(Y.T), np.sign(X)==np.sign(Y)), self.sigma_0,0.),1)[:,None]
    ...
    def diag(self, X):
        ...
        K_diag : ndarray of shape (n_samples_X,)
            Diagonal of kernel k(X, X).
        """
        return np.copy(X[:,0]) * self.sigma_0
...
def plot_gpr_samples(gpr_model, n_samples, ax, transform = no_trans):
    ...
    y_mean = transform(x, y_mean)
    y_samples = transform(x, y_samples)
...
gpr_min_t = GaussianProcessRegressor(kernel=MinT(sigma_0=0.001), random_state=None, n_restarts_optimizer=5)


fig, axs = plot.subplots(nrows=2, sharex=True, sharey=True, figsize=(10, 8))

# plot prior
plot_gpr_samples(gpr_min_t, n_samples=n_samples, ax=axs[0])
axs[0].set_title("Samples from prior distribution")

# plot posterior
gpr_min_t.fit(t_train.reshape(-1, 1), y_train)

y_mean, y_std = gpr_min_t.predict([[0]], return_std=True)
print(y_mean, y_std)
plot_gpr_samples(gpr_min_t, n_samples=n_samples, ax=axs[1])
axs[1].scatter(t_train.reshape(-1, 1), y_train, color="red", zorder=10, label="Observations")
axs[1].legend(bbox_to_anchor=(1.05, 1.5), loc="upper left")
axs[1].set_title("Samples from posterior distribution")
