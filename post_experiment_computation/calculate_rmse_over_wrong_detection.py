...
path = "/home/bela/Cloud/code/Git/alvsts-experiments/eval/wrong_detection/eval"
...
def dir_worker(dirpath, dir: str):
    if dir.startswith("wrong_detection"):
        wrong_detection_str = dir.lstrip("wrong_detection")
...
def plot_rmse_over_wrong():
    ...
    plot.xlabel("fraction of wrong detections")
    plot.ylabel("rmse")

plot_rmse_over_wrong()
save(name="Impact of wrong detection ratio on rmse", path=path)
...
save(name="Impact of wrong detection ratio on measurement count",path=path)
