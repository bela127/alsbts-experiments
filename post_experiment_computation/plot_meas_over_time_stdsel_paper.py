...
colors = plot_meas_over_time(ax, meas_res1_10, exp_quant_name=r"$\mathbf{v_{target}}$")

ax.set_title(r"Measurements Required by \textbf{DEAL} over Time for different $\mathbf{v_{target}}$")
plt.xlim(0,1000)
plt.ylim(0.1,1)
plt.legend()
...
fig, ax = create_fig(subplots=(1,1), width="paper", fraction=1/2)
colors = plot_meas_over_time(ax, meas_res1_10_20, exp_quant_name=r"$\mathbf{v_{target}}$")

ax.set_title(r"Measurement Distribution over Time of \textbf{DEAL} according to LARGE $v_{target}$")
plt.legend()
...
ax.plot(exp_quant, est_meas_res, label=r"Fitted Relation $\mathbf{\hat{m}_{su} = \frac{a + d}{1 + (b*v_{target})^c} - d}$", linestyle='dotted', linewidth=0.5, color='black')
plot_meas_per_step_vs_exp_quant(ax, meas_res, exp_quant_name=r"$\textbf{DEAL}\:\mathbf{v_{target}\in[10^{-2,5}, 10^{-1}]}$", color=appr_style["BR"][0])
